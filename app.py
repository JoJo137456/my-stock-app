...
def check_password():
    ...
    st.markdown(
        ...
        <style>
            ...
            [data-testid="stVerticalBlockBorderWrapper"] {
                background-color: rgba(75, 85, 99, 0.85) !important; /* 85%不透明度的灰色 */
                border-radius: 8px !important;
                border: 1px solid rgba(255, 255, 255, 0.3) !important; /* 微微的亮色外緣 */
                box-shadow: 0 10px 40px rgba(0,0,0,0.6) !important;
                padding: 40px 50px !important;
            }
            
            /* 輸入框與下拉選單：白底 + 明顯的深色粗框線 */
            div[data-baseweb="input"] > div, div[data-baseweb="select"] > div {
                background-color: #ffffff !important;
                border: 2px solid #1f2937 !important; /* 強烈的深色外框線 */
    ...
    with col2:
        with st.container(border=True):
            ...
            st.markdown("<div style='color: #FBBF24; font-size: 1rem; margin-bottom: 5px; margin-top: 20px; font-weight: 800; letter-spacing: 1px; text-shadow: 1px 1px 2px rgba(0,0,0,0.8);'>👤 ACCOUNT</div>", unsafe_allow_html=True)
            st.text_input("account", value="聯合稽核總部", label_visibility="collapsed")
            
            st.markdown("<div style='color: #FBBF24; font-size: 1rem; margin-bottom: 5px; margin-top: 20px; font-weight: 800; letter-spacing: 1px; text-shadow: 1px 1px 2px rgba(0,0,0,0.8);'>🔒 PASSWORD</div>", unsafe_allow_html=True)
