...
# === 5. 左側選單 & 深度戰略連動註解庫 ===
# ==========================================
# 結構：{指標名稱: {"exp": 核心定義, "up": 漲的影響, "down": 跌的影響}}
MACRO_IMPACT = {
    "🇹🇼 台灣加權指數": {
...
if selected_category == "📈 總體經濟與大盤 (宏觀指標)" and option in MACRO_IMPACT:
    ...
    st.markdown(f"""
    <div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; box-shadow: 0 4px 15px rgba(0,0,0,0.04); margin-bottom: 25px;">
        <div style="display: flex; align-items: center; margin-bottom: 20px; border-bottom: 1px solid #f1f5f9; padding-bottom: 16px;">
            <div style="background: #0f172a; color: white; padding: 6px 14px; border-radius: 6px; font-weight: bold; font-size: 14px; margin-right: 12px; letter-spacing: 1px;">AI 戰略洞察</div>
            <div style="font-size: 18px; font-weight: 700; color: #334155;">{option} 連動解析</div>
            <div style="margin-left: auto; color: #94a3b8; font-size: 14px; font-weight: bold;">🔖 收藏參數</div>
        </div>
        
        <div style="margin-bottom: 24px;">
            <div style="font-size: 13px; color: #64748b; font-weight: 700; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 8px; display: flex; align-items: center;">
                <span style="font-size: 16px; margin-right: 6px;">📖</span> 核心定義
            </div>
            <div style="font-size: 15px; color: #334155; line-height: 1.6; background: #f8fafc; padding: 12px 16px; border-radius: 6px; border-left: 3px solid #cbd5e1;">
                {impact_data['exp']}
            </div>
        </div>
        
        <div style="display: flex; gap: 20px; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 300px; background: #fef2f2; padding: 18px; border-radius: 8px; border-left: 4px solid #ef4444;">
                <div style="font-size: 15px; color: #b91c1c; font-weight: 800; margin-bottom: 10px; display: flex; align-items: center;">
                    <span style="font-size: 18px; margin-right: 6px;">🔺</span> 報價上漲對「遠東集團」之影響
                </div>
                <div style="font-size: 14.5px; color: #475569; line-height: 1.6; font-weight: 500;">
                    {impact_data['up']}
                </div>
            </div>
            
            <div style="flex: 1; min-width: 300px; background: #f0fdf4; padding: 18px; border-radius: 8px; border-left: 4px solid #22c55e;">
                <div style="font-size: 15px; color: #15803d; font-weight: 800; margin-bottom: 10px; display: flex; align-items: center;">
                    <span style="font-size: 18px; margin-right: 6px;">🔻</span> 報價下跌對「遠東集團」之影響
                </div>
                <div style="font-size: 14.5px; color: #475569; line-height: 1.6; font-weight: 500;">
                    {impact_data['down']}
                </div>
...
if is_tw_stock:
    ...
    if not df_quarterly.empty and len(df_quarterly) >= 2:
        ...
        with c_chart2:
            ...
            fig2.update_layout(title=f"<b>💰 獲利結構拆解 (最新季度: {latest['季度']})</b>", height=380, margin=dict(l=0, r=0, t=50, b=0), paper_bgcolor='#ffffff',
