...
def fetch_twse_history_proxy(stock_code):
    try:
        data_list = []
        now = datetime.now()
        for i in range(12):  # 擴展至12個月以支援近1年報酬率計算
...
def fetch_us_history(ticker_symbol):
    try:
        ...
        hist = tk.history(period="1y")  # 擴展至1年以支援近1年報酬率計算
...
# === 新增：全標的專業定義庫（指標組成與應用場域） ===
TARGET_DEFINITIONS = {
    **MACRO_IMPACT,
    "👕 1402 遠東新": "遠東新世紀股份有限公司（股票代碼：1402）為遠東集團核心化學纖維製造企業，主要從事聚酯纖維、PET瓶片及非鐵金屬業務。其股價指標用於評估台灣紡織上游產業景氣、原料成本波動及集團製造部門之營運效能。",
    "🏗️ 1102 亞泥": "亞洲水泥股份有限公司（股票代碼：1102）係台灣水泥製造領導企業，業務涵蓋水泥生產與銷售。其股價作為基礎建設產業指標，用於量化國內營建需求、原物料價格及公共工程投資循環。",
    "🚢 2606 裕民": "裕民航運股份有限公司（股票代碼：2606）專營國際散裝貨運業務。其股價指標用於追蹤全球海運運價、原物料貿易量及航運業供給需求平衡狀態。",
    "🧵 1460 宏遠": "宏遠興業股份有限公司（股票代碼：1460）為台灣聚酯纖維製造商。其股價反映紡織纖維產業供需動態及出口導向製造業之競爭力。",
    "🛍️ 2903 遠百": "遠東百貨股份有限公司（股票代碼：2903）經營大型百貨零售業務。其股價指標用於評估台灣內需消費市場動能、零售業績表現及消費者信心指數。",
    "📱 4904 遠傳": "遠傳電信股份有限公司（股票代碼：4904）為台灣行動通訊與固定網路服務供應商。其股價作為電信產業基準，用於分析5G投資回報、數位轉型及通訊基礎設施需求。",
    "🧪 1710 東聯": "東聯化學股份有限公司（股票代碼：1710）專注於石化與化學產品製造。其股價指標用於觀測化工產業原料價格傳導及下游工業需求。",
    "🏦 2845 遠東銀": "遠東國際商業銀行股份有限公司（股票代碼：2845）提供商業銀行金融服務。其股價用於評估金融業資產負債管理、利率敏感度及信貸風險暴露。",
    "🇺🇸 Nike": "Nike, Inc.（NKE）為全球運動鞋類、服飾及設備設計製造領導企業。其股價指標用於衡量運動休閒消費需求、品牌價值及國際供應鏈效能。",
    "🇺🇸 Under Armour": "Under Armour, Inc.（UAA）係運動服飾與配件製造商。其股價反映北美運動服飾市場競爭格局及消費者偏好變化。",
    "🇺🇸 Lululemon": "Lululemon Athletica Inc.（LULU）專營瑜伽及運動服飾。其股價用於評估高端運動服飾產業成長及女性消費趨勢。",
    "🇺🇸 Coca-Cola": "The Coca-Cola Company（KO）為全球碳酸飲料及飲料產品領導製造商。其股價作為消費必需品指標，用於分析品牌忠誠度、通膨影響及全球飲料市場需求。",
    "🇺🇸 PepsiCo": "PepsiCo, Inc.（PEP）經營飲料與休閒食品業務。其股價指標用於觀測食品飲料產業獲利穩定性及消費者生活型態變化。"
}
# ==========================================
...
if period_returns:
    ...
    return_html = '<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 12px; margin-bottom: 20px;">'
    for label, ret in period_returns.items():
        ...
        return_html += f'''
        <div style="background:#ffffff; padding:18px; border-radius:10px; text-align:center; box-shadow:0 2px 5px rgba(0,0,0,0.03); border:1px solid #f1f5f9;">
            <div style="font-size:13px; color:#64748b; margin-bottom:4px;">{label}</div>
            <div style="font-size:22px; font-weight:700; color:{color};">{disp_val}</div>
        </div>
        '''
    return_html += '</div>'
...
st.markdown("""
<div style="background-color: #ffffff; padding: 25px; border-radius: 8px; border-left: 6px solid #ef4444; margin-bottom: 20px;">
<b>判斷標準（專業風險門檻）</b><br><br>
• 近3天報酬率 < -3%：短期價格波動風險提升，潛在受事件驅動。<br>
• 近1週報酬率 < -5%：週線動能轉弱，建議監控成交量變化。<br>
• 近2週報酬率 < -7%：雙週累積跌幅達警示線。<br>
• 近1月報酬率 < -10%：月線趨勢下滑，反映中短期壓力。<br>
• 近1季報酬率 < -15%：季內績效衰退，需檢視基本面。<br>
• 近半年報酬率 < -20%：半年累計回檔顯著，警戒資金動向。<br>
• 近1年報酬率 < -30%：年度報酬低迷，觸發長期審視機制。
</div>
""", unsafe_allow_html=True)
if period_returns:
    thresholds = {
        "近3天": -3, "近1週": -5, "近2週": -7, "近1月": -10,
        "近1季": -15, "近半年": -20, "近1年": -30
    }
    triggered = []
    for label, ret in period_returns.items():
        if isinstance(ret, float) and ret < thresholds.get(label, 0):
            triggered.append(f"{label}：{ret:+.2f}%（低於 {thresholds[label]}% 門檻）")
    if triggered:
        alert_box = f"""<div class="fraud-box-warn">
<div style="font-weight:700; font-size:16px;">⚠️ 目前觸發報酬率警示</div>
<ul>{"".join(f"<li>{item}</li>" for item in triggered)}</ul>
</div>"""
        st.markdown(alert_box, unsafe_allow_html=True)
    else:
        st.markdown('<div class="fraud-box-safe">🟩 所有期間報酬率均未觸發警示門檻</div>', unsafe_allow_html=True)
# 🚀 插入防彈級定義卡片（通用版：標的定義與量化意涵）
