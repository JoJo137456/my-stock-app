        ...
        try:
            ...
            for sheet_name, df in dfs.items():
                ...
                col_mapping = {
                    ...
                    '營業收入淨額': 'revenue', '營業成本': 'cogs', '營業毛利': 'gross_profit',
                    '稅前息前淨利': 'pre_tax_profit', '常續性稅後淨利': 'net_profit',
                    '稅後淨利': 'net_profit', '淨利': 'net_profit',
                    '每股盈餘(元)': 'eps', 'EPS': 'eps',
                    '平均收帳天數': 'ar_days', '平均售貨天數': 'inv_days',
                    '存貨週轉率（次）': 'inv_turnover_times',
                    '存貨': 'inventory', '應收帳款及票據': 'ar_notes',
                    '其他應收款': 'ar_other', '資產總額': 'total_assets',
                    '股東權益總額': 'equity',
                }
                ...
                for col in ['revenue', 'cogs', 'gross_profit', 'pre_tax_profit', 'net_profit',
                            'inventory', 'ar_notes', 'ar_other', 'total_assets', 'equity']:
...
# === 3. 完整 market_categories（已展開，解決錯誤關鍵）===
market_categories = {
    "📈 總體經濟與大盤 (宏觀指標)": {
...
# === 4. API 與圖表函數（保持不變）===
# （為了篇幅這裡省略 fetch_twse_history_proxy、fetch_us_history、get_intraday_chart_data、plot_daily_k、plot_intraday_line）
# 請直接使用你原本的這幾個函數，或告訴我我再補上完整版
...
with st.sidebar:
    ...
    st.markdown("---")
    selected_category = st.selectbox("板塊分類", list(market_categories.keys()))   # ← 現在已不會出錯
    st.markdown("---")
...
# === 其餘程式碼（價格顯示、圖表、TEJ 財務健檢）請接續你原本的內容 ===
# （包含 real_data、hist_data、df_daily、df_intra、價格卡片、TEJ 下半部全部保留）

# ...（請把你原本程式碼中從「# === 價格顯示...」到最後的所有內容貼在這裡）
